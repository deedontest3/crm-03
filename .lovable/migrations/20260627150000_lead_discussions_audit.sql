-- Stage audit slice 1 — Lead, Discussions, Qualified
-- Adds Discussions next-step fields and tightens trigger to enforce
-- BU after Lead and next_step from Discussions onward.
--
-- NOTE: queued for application once the core `public.deals` schema
-- has been restored. Was attempted on 2026-06-27 and rejected with
-- `relation "public.deals" does not exist`.
...
BEGIN
  ...
  IF NEW.stage = ANY(discussions_plus_open)
     AND (NEW.next_step IS NULL OR btrim(NEW.next_step) = '') THEN
    RAISE EXCEPTION 'Next Step is required from Discussions onward';
  END IF;
