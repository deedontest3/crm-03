...
DECLARE
  ...
  post_discussions_stages text[] := ARRAY['Qualified','RFQ','Offered','Negotiation','Verbal Approval','Won'];
...
BEGIN
  ...
  -- BU is part of Lead's own required-field set; enforce only past Lead.
  ...
  -- Next Step belongs to Discussions; require it only when moving PAST Discussions.
  -- Entering Discussions itself no longer fails — user fills it before next move.
  IF NEW.stage = ANY(post_discussions_stages)
     AND (NEW.next_step IS NULL OR btrim(NEW.next_step) = '') THEN
    RAISE EXCEPTION 'Next Step is required before moving past Discussions';
  END IF;
