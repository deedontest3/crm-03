...
DECLARE
  ...
  discussions_plus_open text[] := ARRAY['Discussions','Qualified','RFQ','Offered','Negotiation','Verbal Approval'];
  terminal_stages text[] := ARRAY['Lost','Dropped','Hold'];
  competitor_stages text[] := ARRAY['Negotiation','Verbal Approval','Won'];
BEGIN
  -- Lock Won: once a deal is Won it cannot move to any other stage.
  IF TG_OP = 'UPDATE' AND OLD.stage = 'Won' AND NEW.stage <> 'Won' THEN
    RAISE EXCEPTION 'Won deals are closed and cannot be moved to another stage';
  END IF;
  ...
  -- Exit-based: BU required only when leaving Lead for a non-terminal stage.
  IF TG_OP = 'UPDATE'
     AND OLD.stage = 'Lead'
     AND NEW.stage <> 'Lead'
     AND NOT (NEW.stage = ANY(terminal_stages))
  ...
  -- Exit-based: Next Step required only when LEAVING Discussions-or-later
  -- (i.e. the deal was already at Discussions+ and is moving to a different stage).
  IF TG_OP = 'UPDATE'
     AND OLD.stage = ANY(discussions_plus_open)
     AND NEW.stage <> OLD.stage
     AND NOT (NEW.stage = ANY(terminal_stages))
     AND (NEW.next_step IS NULL OR btrim(NEW.next_step) = '') THEN
    RAISE EXCEPTION 'Next Step is required before leaving %', OLD.stage;
  END IF;
