...
BEGIN
  IF (TG_OP = 'INSERT' OR NEW.signed_contract_date IS DISTINCT FROM OLD.signed_contract_date)
     AND NEW.signed_contract_date IS NOT NULL
     AND NEW.signed_contract_date > today THEN
    RAISE EXCEPTION 'Signed contract date cannot be in the future';
  END IF;

  IF (TG_OP = 'INSERT' OR NEW.stage IS DISTINCT FROM OLD.stage OR NEW.bu IS DISTINCT FROM OLD.bu)
     AND NEW.stage = ANY(post_lead_stages)
  ...
  IF (TG_OP = 'INSERT' OR NEW.stage IS DISTINCT FROM OLD.stage OR NEW.next_step IS DISTINCT FROM OLD.next_step)
     AND NEW.stage = ANY(post_discussions_open)
  ...
  IF (TG_OP = 'INSERT'
        ...
        OR NEW.next_step_due_date IS DISTINCT FROM OLD.next_step_due_date)
     AND NEW.stage = ANY(open_stages)
  ...
  IF (TG_OP = 'INSERT'
     ...
     AND NEW.stage = ANY(competitor_stages)
  ...
  IF (TG_OP = 'INSERT' OR NEW.stage IS DISTINCT FROM OLD.stage OR NEW.revise_date IS DISTINCT FROM OLD.revise_date)
     AND NEW.stage = 'Hold'
  ...
  IF (TG_OP = 'INSERT'
        OR NEW.stage IS DISTINCT FROM OLD.stage
        OR NEW.expected_signing_date IS DISTINCT FROM OLD.expected_signing_date
        OR NEW.po_status IS DISTINCT FROM OLD.po_status)
     AND NEW.stage = 'Verbal Approval'
  ...
  IF (TG_OP = 'INSERT'
        OR NEW.rfq_received_date IS DISTINCT FROM OLD.rfq_received_date
        OR NEW.proposal_due_date IS DISTINCT FROM OLD.proposal_due_date)
  ...
  IF (TG_OP = 'INSERT'
        OR NEW.stage IS DISTINCT FROM OLD.stage
        OR NEW.expected_closing_date IS DISTINCT FROM OLD.expected_closing_date)
     AND NEW.stage = ANY(open_stages)
  ...
  IF (TG_OP = 'INSERT'
        OR NEW.stage IS DISTINCT FROM OLD.stage
        OR NEW.implementation_start_date IS DISTINCT FROM OLD.implementation_start_date
        OR NEW.expected_signing_date IS DISTINCT FROM OLD.expected_signing_date)
     AND NEW.stage = 'Verbal Approval'
  ...
  IF (TG_OP = 'INSERT'
        OR NEW.stage IS DISTINCT FROM OLD.stage
        OR NEW.signed_contract_date IS DISTINCT FROM OLD.signed_contract_date
        OR NEW.rfq_received_date IS DISTINCT FROM OLD.rfq_received_date)
     AND NEW.stage = 'Won'
     AND NEW.signed_contract_date IS NOT NULL
     AND NEW.rfq_received_date IS NOT NULL
